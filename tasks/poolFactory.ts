...
task(
    "createPool",
    "Create a new Pool from the PoolFactory"
)
    ...
    .addParam("withdrawGateBps", "Bips, percent of liquidity pool available to withdraw")
    .addParam("serviceFeeBps", "Bips, percent taken from borrower payments to be paid to pool admin")
    .addParam("firstLossInitialMinimum", "Amount of tokens to be deposited to first loss before a pool is active")
    .addParam("withdrawRequestPeriodDuration", "seconds")
    .addParam("fixedFee", "Amount of tokens the pool admin can claim every interval")
    .addParam("fixedFeeInterval", "Interval in days at which a pool admin can claim fixed fees from the pool")
    .setAction(async (taskArgs, hre) => {
        const PoolFactory: ContractFactory =
            await hre.ethers.getContractFactory("PoolFactory");
        ...
        const poolSettings = {
            ...
            requestCancellationFeeBps: hre.ethers.BigNumber.from(taskArgs.requestCancellationFeeBps),
            ...
            firstLossInitialMinimum: hre.ethers.BigNumber.from(taskArgs.firstLossInitialMinimum),
            withdrawRequestPeriodDuration: hre.ethers.BigNumber.from(taskArgs.withdrawRequestPeriodDuration),
        ...
        const txn = await poolFactory.connect(signer).createPool(
            taskArgs.liquidityAsset,
            poolSettings
        );
