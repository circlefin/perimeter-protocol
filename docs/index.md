...
```solidity
constructor(contract IServiceConfiguration serviceConfiguration, address factory_, address borrower_, address pool_, address liquidityAsset_, struct ILoanSettings settings_) public
```
...
_Reference to the ServiceConfiguration contract_
...
```solidity
function createLoan(address borrower, address pool, address liquidityAsset, struct ILoanSettings settings) public virtual returns (address LoanAddress)
```
...
```solidity
constructor(address liquidityAsset, address poolAdmin, address serviceConfiguration, address withdrawControllerFactory, address poolControllerFactory, struct IPoolConfigurableSettings poolSettings, string tokenName, string tokenSymbol) public
```

_Constructor for Pool_
...
```solidity
constructor(address pool_, address serviceConfiguration_, address admin_, address liquidityAsset_, struct IPoolConfigurableSettings poolSettings_) public
```
...
_Constructor for a Pool's withdraw state_
...
```solidity
function calculateConversion(uint256 input, uint256 numerator, uint256 denominator, bool roundUp) external pure returns (uint256)
```
...
```solidity
constructor(contract IServiceConfiguration serviceConfiguration, address factory, address borrower, address pool, address liquidityAsset_, struct ILoanSettings settings_) public
```

_The constructor for the PermissionedLoan contract. It holds a reference
to the corresponding PermissionedPool's access control contract to enforce the
same controls on borrowers._
...
```solidity
constructor(address liquidityAsset, address poolAdmin, address serviceConfiguration, address withdrawController, address poolController, address poolAccessControlFactory, struct IPoolConfigurableSettings poolSettings, string tokenName, string tokenSymbol) public
```

_The constructor for the PermissionedPool contract. It calls the
...
```solidity
function createPool(address liquidityAsset, struct IPoolConfigurableSettings settings) public returns (address poolAddress)
```
...
```solidity
contract IServiceConfiguration _serviceConfig
```

_ServiceConfiguration_
