...
describe("PermissionedLoan", () => {
    ...
    describe("postFungibleCollateral()", () => {
        it("posts collateral for valid participants", async () => {
            const { loan, tosAcceptanceRegistry, borrower, poolAdmin, poolAccessControl, liquidityAsset } =
                await loadFixture(loadLoanFixture);

            // allow borrower
            await tosAcceptanceRegistry.connect(borrower).acceptTermsOfService();
            await poolAccessControl.connect(poolAdmin).allowParticipant(borrower.address);

            await expect(
                loan
                    .connect(borrower)
                    .postFungibleCollateral(liquidityAsset.address, 0)
    ...
    describe("postNonFungibleCollateral()", () => {
        it("posts collateral for valid participants", async () => {
            const { loan, tosAcceptanceRegistry, borrower, poolAdmin, poolAccessControl, nftAsset } =
                await loadFixture(loadLoanFixture);
