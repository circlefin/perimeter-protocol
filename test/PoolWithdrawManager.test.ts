...
describe("PoolWithdrawManager", () => {
  ...
  async function loadPoolFixture() {
    const [operator, poolAdmin, lenderA, lenderB] = await ethers.getSigners();
    ...
    const withdrawManager = await ethers.getContractAt(
      "PoolWithdrawManager",
    ...
    await liquidityAsset
      .connect(lenderA)
      .approve(pool.address, LENDER_A_DEPOSIT);
  ...
  describe("needsCrank()", () => {
    it("returns true if lender needs crank", async () => {
      ...
      await withdrawManager.crankIncrementally(lenderA.address);
